...
class IosConverterAndroidFragment : BaseFragment() {
    ...
    override fun initData() {
        ...
        converter.setOnClickListener {
            ...
            val json = JSONObject(androidArMap).toString()

            val sdDir = Environment.getExternalStorageDirectory()
            val sdpath: String = sdDir.absolutePath
            val filepath = sdpath + File.separator.toString() + "android_ar.json"
            val result = FileIOUtils.writeFileFromString(filepath, json)
            Log.d(TAG, "initData: 写入成功 result $result, 文件: ${File(filepath).exists()} , 路径 ${filepath}")
        }
    }

    private fun writeJson(androidArMap: MutableMap<String, String>): String {
        val gson = Gson()
        val jsonTree = gson.toJsonTree(androidArMap, MutableMap::class.java)
    ...
    private fun changeSingleText(
        ...
        for ((adCnKey, adCnValue) in androidCnMap) {
            if (iosCn == adCnValue && androidArMap[adCnKey] !=iosAr) {
                val awaitValue = androidArMap[adCnKey]
                awaitCount++
                Log.d(TAG, "可以修改的数据: $awaitCount 带修改的文案: awaitValue: $awaitValue => 他的中文: $adCnKey 正确的文案: $iosAr")
